...
var health_scale
var dmg_scale
var dmg = round(rand_range(10, 65))
#State variables
...
func lookout(degree):
	...
	if aggro_timer < 400 or see_player and degree == 1:
		aggroed = true
		speed = AGGRO_SPEED 
		$RayCast.cast_to.y = -45
		$RayCast2.cast_to.y = 45
		$SkyRay.cast_to.z = 40
		$HitBox/HitBox.scale *= 1.25
		
	if aggro_timer < 500 or see_player and degree == 2:
		aggroed = true
		speed = AGGRO_SPEED * 1.1
		$RayCast.cast_to.y = -50
		$RayCast2.cast_to.y = 50
		$SkyRay.cast_to.z = 45
	...
	if see_player and degree == 3:
		...
		$RayCast.cast_to.y = -57
		$RayCast2.cast_to.y = 57
		$SkyRay.cast_to.z = 52
		$HitBox/HitBox.scale *= 1.75
func _ready():
